...
class TestCone(unittest.TestCase):
    ...
    def test_1(self):
        g = {"$id":"1","neighbors":[{"$id":"2","neighbors":[{"$ref":"1"},{"$id":"3","neighbors":[{"$ref":"2"},{"$id":"4","neighbors":[{"$ref":"3"},{"$ref":"1"}],"val":4}],"val":3}],"val":2},{"$ref":"4"}],"val":1}
        graph = self.builder.build(g)
        result = self.func.cloneGraph2(graph)
        labels = [None]*5
        for n in result.neighbors:  # graph.label = 1, labels covers 2, 4
            labels[n.label] = n
